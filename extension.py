...
def PCA(X, Y):
    ...
    U   = pca.components_
    X_reduced = np.matmul(X, U.T)
    kmeans = sklearn.cluster.KMeans(n_clusters=4, n_init=10).fit(X_reduced)
    labels = kmeans.labels_
    ind1 = np.where(labels==0)[0]
    ind2 = np.where(labels==1)[0]
    ind3 = np.where(labels==2)[0]
    ind4 = np.where(labels==3)[0]
    preds = np.hstack((ind1, ind2, ind3, ind4))
    def relu(x):
        return x[x>0]
    eg_class = 1-relu(np.sign(preds)*Y)
    eg_class = eg_class.sum()/float(preds.shape[0])
    
    return eg_class

# Figure 1 with lin sep 
...
PCA_error     = np.zeros((num_sigmas)) 

for i in range(0, num_sigmas):
  X, Y, mu            = make_linsep_GMM(dim = dim_oracle, N=N , var = sigma[i], plot = False)
  labels              = oracle(X, mu)
  PCA_error[i]        = PCA(X, Y)
  ...
  print('Oracle Classifiaction error: {} for sigma: {}'.format(np.round(oracle_error[i], 3),np.round(sigma[i], 3)))
  print('PCA Classifiaction error:    {} for sigma: {}'.format(np.round(PCA_error[i], 3),np.round(sigma[i], 3)))
...
for i in range(0,num_sigmas):
  X_, Y_, mu  = make_linsep_GMM(dim = dim_RF, N=N , var = sigma[i], plot = False)
  ...
  with torch.no_grad():
    ...
    RF_error[i] = eg_class 
    print("Test Data: RF Classification Error: {}; Variance: {}; halfMSE-Loss:{}".format(np.round(RF_error[i], 3),np.round(sigma[i], 3),eg))
    print("---------------------------------------------------------")
...
######################################################################
for i in range(0,num_sigmas):
  X, Y, m   = make_linsep_GMM(dim = dim_NN, N = N, var = sigma[i],plot=False) # Generate new data with new variance
  X_train, X_val, Y_train, Y_val = make_splits(X, Y)
  X_train   = X_train
  Y_train   = Y_train
  X_val     = (X_val).float()
  ...
  with torch.no_grad():
    ...
    eg_class = eg_class.sum()/float(preds.shape[0])
    NN_error[i] = eg_class 
    print("Test Data: NN Generalized Classification Error: {}; Variance: {}; Loss:{}".format(np.round(NN_error[i], 3),np.round(sigma[i], 3),eg))
    print("---------------------------------------------------------")
